...
namespace HcUtils
{
    class RagdollWizard : MonoBehaviour
    {

        [SerializeField] private string pelvisName = "Hips";
        [SerializeField] private string leftHipsName = "LeftUpLeg";
        [SerializeField] private string leftKneeName = "LeftLeg";
        [SerializeField] private string leftFootName = "LeftFoot";
        [SerializeField] private string rightHipsName = "RightUpLeg";
        [SerializeField] private string rightKneeName = "RightLeg";
        [SerializeField] private string rightFootName = "RightFoot";
        [SerializeField] private string leftArmName = "LeftArm";
        [SerializeField] private string leftElbowName = "LeftForeArm";
        [SerializeField] private string rightArmName = "RightArm";
        [SerializeField] private string rightElbowName = "RightForeArm";
        [SerializeField] private string middleSpineName = "Spine2";
        [SerializeField] private string headName = "Head";
        ...
        {
            pelvis = FindTransformWithName(pelvisName);

            leftHips = FindTransformWithName(leftHipsName);
            leftKnee = FindTransformWithName(leftKneeName);
            leftFoot = FindTransformWithName(leftFootName);

            rightHips = FindTransformWithName(rightHipsName);
            rightKnee = FindTransformWithName(rightKneeName);
            rightFoot = FindTransformWithName(rightFootName);

            leftArm = FindTransformWithName(leftArmName);
            leftElbow = FindTransformWithName(leftElbowName);

            rightArm = FindTransformWithName(rightArmName);
            rightElbow = FindTransformWithName(rightElbowName);

            middleSpine = FindTransformWithName(middleSpineName);
            head = FindTransformWithName(headName);
        }

        private Transform FindTransformWithName(string name)
        {
            var transforms = GetComponentsInChildren<Transform>(true);
            foreach (Transform child in transforms)
            {
                if (child.gameObject.name.Contains(name)) return child;
            }
            return null;
        }

        [ContextMenu("Create Ragdoll")]
        public void CreateRagdoll() => OnWizardCreate();
        void BuildBodies()
        {
            foreach (BoneInfo bone in bones)
            {
